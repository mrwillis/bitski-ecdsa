...
import {providers, Contract as ethersContract, utils} from "ethers"
...
export default class App {
  ...
  constructor() {
    ...
    this.bitskiEthersWeb3Provider = new providers.Web3Provider(this.web3.currentProvider);
    // Initialize the sample contract
    this.contract = new Contract(this.web3, artifacts);
    this.myContractEthersWrapper = new ethersContract("0xd994F221a2680f49013cD6321E27926ba6e7F293", artifacts.abi, this.bitskiEthersWeb3Provider);
    console.log(this.myContractEthersWrapper)
  }
  ...
  configureView() {
    ...
    this.signPayloadMetamaskElement = document.getElementById('sign-payload-metamask');
    this.signPayloadBitskiElement = document.getElementById('sign-payload-bitski');
  ...
  signPayloadMetamask() {
    window.ethereum.enable().then(() => {
      ...
      signer.getAddress().then((address) => {
        signer.provider.send("personal_sign", [
          hash,
          address
        ]).then((signature) => {
          console.log(`Metamask signature: ${signature}`)
          this.myContractEthersWrapper.computeRecoveryAddress(hash, signature).then((address) => {
